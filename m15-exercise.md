...
GitHub Skills「Connect the dots」の日本語訳です。演習そのものは英語で、自分のリポジトリの **README** に
...
GitHub には、GitHub 上の別の情報を参照するための特別な機能があります。たとえば、別の Issue や pull request を番号で参照すると、その番号がリンクになります。同時に、リンク先の Issue や pull request の側にも相互参照（cross-reference）が作られます。この双方向の参照によって、GitHub 上の情報どうしの関係を追えます。
...
バージョン管理の重要な点のひとつは、過去をさかのぼって見られることです。`git blame` を使ってコミットの背景をたどると、コードについて人を _blame_（責める）する以上のことができます。なぜそのコミットが行われたのかという経緯が見えます。関連する pull request はどれか。誰がその pull request を承認したか。マージ前にそのコミットでどんなテストが実行されたか。

履歴を調べる分かりやすい理由は、履歴を知るためです。Issue と pull request があれば、最低限の情報だけでなく、より完全な経緯が分かります。

### `git blame` とは（What's `git blame`?）

`git blame` は、ファイルの各行を最後に変更したリビジョンと作者を表示する Git の機能です。誰がいつコミットしたか、さらになぜコミットしたかまで、この方法で調べられます。ファイルへの変更を誰が入れたのか分からないときは、`git blame` で確認できます。`git blame` という名前は責任追及のように聞こえますが、判断の背景を理解するために使えます。

### SHA（Secure Hash Algorithm）とは（What's a Secure Hash Algorithm (SHA)?）

SHA は特定のオブジェクトへの参照です。ここではコミットへの参照を指します。GitHub では、特定のコミットを開くと、どんな変更が入ったか、誰が入れたか、pull request の一部だったかを確認できます。
...
これで、サイドバーが確かに追加されたこと、そしてそれがあのコミットで行われたことが分かりました。もう少し掘り下げて、この変更をめぐる計画や会話がコメントとして残っていないか調べましょう。
...
pull request が重要なのは、コミットがいつ行われたかを超えた情報が分かるからです。_なぜ_ そのコミットが行われたかを知ることができます。履歴を調べるのは誰かを _責める_ ためではなく、全体像を見るためです。なぜその判断がされたのか。誰が関わったのか。各コミットのビルド結果とテスト結果はどうだったのか。誰が変更を要求し、誰が承認したのか。

### コミットから pull request を見つける（Finding a pull request from a commit）

GitHub でコミットを表示すると、多くの情報が見られます。この画面から、そのコミットが作られた pull request へのリンクもたどれます。次の Step でこれを使います。
...
このコースでは、情報を見つけて共有する方法を学びました。GitHub のリポジトリの中では、どんな変更が行われたかの履歴と、さらに重要な「なぜその変更が行われたか」を調べられます。
...
5. **Save** ボタンをクリックします。
6. 30 秒ほど待ってからページを再読み込みします。"Your site is published at ..." と表示されたら、そのリンクから公開されたサイトを見られます。

さらに学ぶ・参加するための情報です。

- 学生の方は [Student Developer Pack](https://education.github.com/pack) を確認してください。
- このコースの感想は [discussion board](https://github.com/orgs/skills/discussions/categories/connect-the-dots) で聞かせてください。

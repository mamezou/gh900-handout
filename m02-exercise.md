...
**ブランチとは**: リポジトリの「並行するバージョン」です。リポジトリには最初 `main` という 1 本のブランチがあり、正式版とみなされます。ブランチを追加すると、`main` のコピーの上で、本体に影響を与えずに安全に変更できます。多くの人は、特定の機能に取り組むときにブランチを使います。ブランチがあるので、作業している間も全員の成果は安全に保たれます。

**プロフィール README とは**: GitHub のプロフィールページの上部に表示される「自己紹介」です。

### ⌨️ やること：最初のブランチ

1. ブラウザーで新しいタブを開き、さっき作った自分のリポジトリ（演習のコピー）を開きます。説明は元のタブで読み、操作は新しいタブで行います。
...
4. **Find or create a branch...** の入力欄に `my-first-branch` と入力します。
   ...
   > **注**: ブランチ名が次の Step へ進む条件としてチェックされます。
...
6. ブランチが GitHub に作られたので、Mona が作業を確認しています。少し待って、Issue のコメントを見てください。進捗と次の Step が投稿されます。
...
コミットは、ファイルの追加・削除・名前変更や中身の変更を記録します。今回は「新しいブランチに新しいファイルを追加する」がコミットになります。
...
プロジェクトに変更を加えてコミットしたので、次は pull request で「変更を取り込んでほしい」と提案します。

**pull request とは**: 共同作業は pull request の上で行われます。pull request は自分のブランチの変更を他の人に見せ、受け入れる・却下する・追加の変更を提案する、ができます。今回の pull request は、自分のブランチで行った変更を `main` に適用する提案になります。
...
**Compare & pull request** を押すと自動で作成画面に進めます（押した場合は下の 5 へ）。練習のため、1〜4 の手動手順でも作れます。
...
6. 次の欄は変更の**説明**です。作業した内容（新しいブランチを作った、ファイルを作った、コミットした）を短く書きます。
...
2. **Confirm merge** をクリックします。
   ...
   > **ヒント**: 確認のダイアログ、ファイルを追加したときと似ていませんか？ マージもコミットの一種です。

...
GitHub Skills「Review pull requests」の日本語訳です。演習そのものは英語で、自分のリポジトリの **Issues** に
Step 1 から順に表示されます。画面の英語とこの訳を見比べながら進めてください。

- ボタン名、ファイル名、ブランチ名、pull request のタイトル、コミットメッセージは、画面と一致させるため英語のままにしています。**指定された名前はそのまま入力してください。** 自動チェックはその名前を見ています。
...
- **Checks** — GitHub Actions による自動処理を pull request に適用したときの結果です。これは別の演習で扱います。😎
- **Files Changed** — 変更前後を見比べられる差分（Diff）の表示です。その場でコメントやレビューを追加することもできます。
...
1. ブラウザーで新しいタブを開き、このリポジトリを開きます。説明はこのタブで読み、操作はもう一方のタブで行います。
...
**pull request の担当者（assignee）** は、その変更内容をいちばんよく知っている人のことです。質問があるときに誰へ連絡すればよいかを把握しておくための、簡単なしくみです。
...
- **Approve** — ルールセット、コードオーナー、その他のポリシーが適用されている場合に、マージを可能にします。
...
4. レビューの要件、テスト、品質確認などについては、そのリポジトリのコントリビューションガイドを参照します。

#### レビューの観点

- 潜在的な問題・リスク・制約を見つける。
- 変更や改善を提案する。
- この pull request が考慮していない、今後の変更について知らせる。
- 認識が合っているかを確かめる質問をする。
- 作成者がうまくやれている点、これからも続けてほしい点を挙げる。
- 重要なフィードバックから優先して伝える。
- 簡潔に、かつ意味のある詳しさで書く。
...
**Add a suggestion** は、コメント入力欄にあるボタンです。押すと、専用の書式のコードブロックが挿入されます。GitHub は、コメントを表示するだけでなく **Commit suggestion** ボタンも表示します。これにより、作成者はボタンひとつで提案を受け入れてコミットできます。コードエディターを開く必要はありません。便利ですね！
...
4. プラスのアイコンをクリックすると、その行へのコメント入力欄が開きます。

5. **Add a suggestion** ボタンをクリックすると、その行を編集できる形で挿入されます。
